...
class GMK_ApproximateQPlayer(Player):
    def __init__(self, letter, size=15, transfer_player=GMK_Intermediate):
        ...
        self.weights["#-of-unblocked-five-player"] = 1000
        self.weights["#-of-unblocked-three-opponent"] = -500
        self.weights["#-of-unblocked-three-player"] = 30
        self.weights["#-of-unblocked-four-opponent"] = -1000
        self.weights["#-of-unblockable-four-player"] = 70
    ...
    def update_rewards(self, reward: float):
        ...
        * Make a call to update_q_values() for each state-action pair in the action history.
        """
        for _iter in range(len(self.action_history) - 1, -1, -1):
            state, action = self.action_history[_iter]
            if _iter == len(self.action_history) - 1:
                next_state = None
            else:
                next_state, _ = self.action_history[_iter + 1]
    ...
    def update_q_values(self, state, action, next_state, reward):
        ...
        for feature_name, feature_value in features.items():
            self.weights[feature_name] += (
                self.learning_rate
                * (reward + self.gamma * max_q_next - q_state_action)
                * feature_value
            )
    ...
    def feature_vector(self, state, action) -> DefaultDict[str, float]:
        ...
        return self.feature_extractor.get_features(
            copy.deepcopy(state), action, self.letter
        )
...
class FeatureExtractor(ABC):
    @abstractmethod
    def get_features(
        self, state: List[List[str]], move: Union[List[int], Tuple[int]], player: str
...
class SimpleExtractor(FeatureExtractor):
    def get_features(self, state, move, player):
        ...
        feats = defaultdict(float)
        feats["#-of-unblocked-three-player"] = self.number_of_open_threes(player, state)
        feats["#-of-unblocked-three-opponent"] = self.number_of_open_threes_op(opponent, state)
        feats["#-of-unblocked-four-opponent"] = self.number_of_unblock_fours(opponent, state)
        feats["#-of-unblocked-five-player"] = self.number_of_unblock_fives(player, state)
        feats["#-of-unblockable-four-player"] = self.number_of_potential_wins(player, state)
        feats["bias"] = 1.0
    ...
    def number_of_open_threes(self, player, board):
        length = 5

        def check_open_three(player, array):
            lst = list(array)
            return (
                lst[1:4].count(player) == 3
                and lst.count(None) + lst[0:1].count(player) + lst[4:5].count(player)
                == 2
            )
    ...
    def number_of_unblock_fours(self, player, board):
        length = 5
    ...
    def number_of_unblock_fives(self, player, board):
        length = 5
    ...
    def count_cross_pattern(self, player, board):
        length = 5
        ...
        for row in range(size):
            for col in range(size):
                open_three_count = 0
                if col <= size - length:
                    if check_open_three(player, board[row, col : col + length]):
                        open_three_count += 1
                if row <= size - length:
                    if check_open_three(player, board[row : row + length, col]):
                        open_three_count += 1
    ...
    def number_of_unblock_fours_me(self, player, board):
        length = 5

        def check_open_four(player, array):
            lst = list(array)
            return (lst[0:4].count(player) == 4 and lst.count(None) == 1) or (
                lst[1:5].count(player) == 4 and lst.count(None) == 1
            )
    ...
    def number_of_potential_wins(self, player, board):
        length = 6

        def check_open_four(player, array):
            lst = list(array)
            return lst[1:5].count(player) == 4 and lst.count(None) == 2
    ...
    def number_of_open_threes_op(self, player, board):
        length = 6
        ...
        def check_open_three(player, array):
            ...
            return (
                (
                    lst[1:4].count(player) == 3
                    and lst.count(None)
                    + lst[0:1].count(player)
                    + lst[4:6].count(player)
                    == 3
                )
                or (
                    lst[2:5].count(player) == 3
                    and lst.count(None)
                    + lst[0:2].count(player)
                    + lst[5:6].count(player)
                    == 3
                )
                or (
                    lst[1:3].count(player) == 2
                    and lst.count(None) == 3
                    and lst[4:5] == player
                )
                or (
                    lst[1:2].count(player) == 1
                    and lst.count(None) == 3
                    and lst[3:5].count(player) == 2
                )
            )
