...
class TicTacToe(Game):
        ...
        for i in range(height):
            ...
            for j in range(width):
                if self.board_state[i][j] == None:
                    ...
                    raise ValueError(
                        f"Invalid value {self.board_state[i][j]} in the board"
                    )
            print("|")
        print("-------------")
    ...
    def set_move(self, x, y, player_letter):
        assert (
            self.curr_player == player_letter
        ), f"Invalid player {player_letter}. Current player is {self.curr_player}"
...
class Gomoku(Game):
    ...
    def not_empty_cells(self, state=None):
        if state is None:
            state = self.board_state  # Use current state if not provided
    ...
    def valid_move(self, x, y):
        return (
            0 <= x < self.size and 0 <= y < self.size and self.board_state[x][y] == None
        )
    ...
    def wins(self, player_letter, state=None):
        ...
        for axis in directions:
            ...
            for xdirection, ydirection in axis:
                axis_count += self.direction_count(
                    last_x, last_y, xdirection, ydirection, player_letter, state
                )
            if axis_count >= 5:
    ...
    def direction_count(self, x, y, xdirection, ydirection, letter, state):
        ...
        for step in range(1, 5):  # look four more steps on a certain direction
            if xdirection != 0 and (
                x + xdirection * step < 0 or x + xdirection * step >= self.size
            ):
                break
            if ydirection != 0 and (
                y + ydirection * step < 0 or y + ydirection * step >= self.size
            ):
                break
        ...
        for xdirection, ydirection in axis:
            for step in range(1, 5):
                ...
                if ydirection != 0 and (
                    ...
                    break
                if (
                    self.board_state[x + xdirection * step][y + ydirection * step]
